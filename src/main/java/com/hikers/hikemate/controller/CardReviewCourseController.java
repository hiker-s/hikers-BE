...
@RequestMapping("/api/course/review/{course_id}")
public class CardReviewCourseController {

    private final JwtUtil jwtUtil;
    private final CardReviewService courseReviewService;

    @GetMapping()
    public ResponseEntity<?> getReviewListByCourse(@RequestHeader(value = "Authorization", required = false) String token,
                                           @RequestParam(value = "sortType", defaultValue = "latest") String sortType,
                                           @PathVariable Long course_id
    ) {
        try {
            User user = jwtUtil.getUserFromToken(token);
            List<CardReviewDTO> reviewPostList = courseReviewService.getReviewListByCourse(sortType, user.getUserId(), course_id);

            return ResponseEntity.status(HttpStatus.OK).body(new SuccessResponseDTO<>(
                    200,
                    ("likes".equals(sortType) ? "좋아요순으로 " : "최신순으로 ") + "정렬된 코스 별 리뷰 조회에 성공하였습니다.",
                    reviewPostList
            ));
